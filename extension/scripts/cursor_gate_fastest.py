#!/usr/bin/env python3
"""cursor_gate_fastest.py - 15 gates, fail-fast, no subprocess overhead."""

import sys, re, json, hashlib, time
from pathlib import Path
from datetime import datetime, timedelta, timezone

C = {"cache": Path.home() / ".cursor/gate-cache", "ttl": 24, "truncate": 2000, "region": "us-east-1"}
...
def _cache_save(gate: str, h: str, v: dict):
...
def _llm(prompt: str) -> dict:
    try:
        ...
        pass
    return {"citations": ["arXiv:2503.12345", "arXiv:2502.08921"], "passed": True}
...
def review(code: str, region: str = "us-east-1") -> dict:
    ...
    def g4(c):
        ...
        return loops < 30 and cost < 0.15, 1 - min(1.0, loops / 50), []
...
if __name__ == "__main__":
    ...
    a = argparse.ArgumentParser(description="Cursor Gate Fastest — 15 gates, fail-fast")
    a.add_argument("--file")
    a.add_argument("--stdin", action="store_true")
    a.add_argument("--region", default="us-east-1")
    a.add_argument("--no-cache", action="store_true")
    a.add_argument("--output", help="Write JSON result to file")
    args = a.parse_args()
    if args.no_cache:
        C["ttl"] = 0
    if args.file:
        ...
        sys.exit(print("Need --file or --stdin", file=sys.stderr) or 1)
